...
pub fn load_editor_workspace(project_root: &Path) -> JaymiResult<Option<EditorWorkspaceSnapshot>> {
    ...
    let body = fs::read_to_string(&path).map_err(|error| {
        JaymiError::new(format!("read {}: {error}", path.display()))
    })?;
    let snapshot: EditorWorkspaceSnapshot = serde_json::from_str(&body).map_err(|error| {
        JaymiError::new(format!("parse {}: {error}", path.display()))
    })?;
    Ok(Some(snapshot))
}
...
pub fn save_editor_workspace(
    ...
    let body = serde_json::to_string_pretty(snapshot).map_err(|error| {
        JaymiError::new(format!("serialize workspace editor state: {error}"))
    })?;
    let tmp = layout.workspace_json.with_extension("json.tmp");
    fs::write(&tmp, &body).map_err(|error| {
        JaymiError::new(format!("write {}: {error}", tmp.display()))
    })?;
...
mod tests {
    ...
    fn roundtrip_omits_need_for_contents() {
        ...
        let loaded = load_editor_workspace(&root).expect("load").expect("present");
