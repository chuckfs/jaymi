...
- **Editor** hierarchy: Tabs (full-width under Quick Action Bar + breadcrumb) → Monaco (full-bleed centerpiece) → Status Bar · Explorer beside Monaco only · bottom dock full-width (Monaco via wry WebView overlay on the focused pane only; egui buffer fallback for every other pane; Save via ⌘S)
  - **Quick Action Bar** (~40pt): Planner-intent strip — Explain · Edit · Refactor · Search · Run · Terminal · Git (plus `{}` close chrome). Not a VS Code toolbar: Explain/Edit/Refactor insert Planner prompts into the conversation composer; Search/Run/Terminal/Git open or focus dock panels. Narrow widths collapse trailing actions into **More**. Close Workspace, Command Palette (⌘P), Panel, and Save remain available via shortcuts / palette / composer.
